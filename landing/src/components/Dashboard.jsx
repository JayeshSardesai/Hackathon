...
const Dashboard = () => {
  ...
  const fetchCropPredictions = async () => {
    try {
      ...
      const response = await fetch('http://localhost:5000/api/crop-prediction', {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
          'Authorization': `Bearer ${token}`,
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 mt-8 mb-8">
            ...
            <a href={`http://127.0.0.1:5001/?lang=${encodeURIComponent(i18n.resolvedLanguage || i18n.language || 'en')}`} className="farm-card p-6 hover:shadow-xl transition-all duration-300 block">
              <div className="flex items-center space-x-2 mb-2">
                <i data-lucide="lightbulb" className="h-5 w-5 text-farm-green-600"></i>
                <h3 className="font-semibold text-farm-green-700 font-poppins"><T k="dashboard.cropRecommendation">Crop Recommendation</T></h3>
              </div>
              <p className="text-sm text-muted-600 font-poppins"><T k="dashboard.getAiSuggestions">Get AI-powered crop suggestions</T></p>
              <div className="mt-4 text-farm-green-600 flex items-center font-poppins">
                <T k="dashboard.tryItNow">Try it now</T> <i data-lucide="arrow-right" className="h-4 w-4 ml-1"></i>
              </div>
            </a>

          {/* Yield Estimation Card */}
            <a href={`http://127.0.0.1:7860/?lang=${encodeURIComponent(i18n.resolvedLanguage || i18n.language || 'en')}`} className="farm-card p-6 hover:shadow-xl transition-all duration-300 block">
              <div className="flex items-center space-x-2 mb-2">
                <i data-lucide="bar-chart-2" className="h-5 w-5 text-golden-yellow-600"></i>
                <h3 className="font-semibold text-golden-yellow-700 font-poppins"><T k="dashboard.yieldEstimation">Yield Estimation</T></h3>
              </div>
              <p className="text-sm text-muted-600 font-poppins"><T k="dashboard.predictYields">Predict your crop yields</T></p>
