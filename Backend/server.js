...
app.post('/api/crop-prediction', authenticateToken, async (req, res) => {
    try {
        ...
        // Call ML model API
        const mlResponse = await axios.post('http://127.0.0.1:5001/', predictionData);
        console.log('🤖 ML model response:', mlResponse.data);

        if (mlResponse.data && mlResponse.data.top_crops) {
            // Convert probabilities to percentages and format for frontend
            const cropPredictions = Object.entries(mlResponse.data.top_crops).map(([crop, probability]) => ({
                crop: crop,
...
async function getCurrentWeatherData() {
    try {
        // Try to get weather from OpenWeatherMap API using default coordinates
        // You can replace these with actual coordinates from user's location
        const defaultLat = 20.5937; // Default to India center
        const defaultLng = 78.9629;
        const apiKey = "c76c099025fdd8cbb647172c0eba0197";

        const response = await axios.get(
            `https://api.openweathermap.org/data/2.5/weather?lat=${defaultLat}&lon=${defaultLng}&appid=${apiKey}&units=metric`
        );
...
app.post('/api/production-estimation', authenticateToken, async (req, res) => {
    try {
        ...
        const mlResponse = await axios.post('http://127.0.0.1:7860/api/predict', estimationData);
...
app.post('/api/production-estimates-all', authenticateToken, async (req, res) => {
    try {
        ...
        const cropResponse = await axios.post('http://127.0.0.1:5001/', predictionData);
