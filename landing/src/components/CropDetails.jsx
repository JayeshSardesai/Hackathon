...
const CropDetails = () => {
  ...
  const loadCropDetailsForLocation = async (state, district) => {
    try {
      ...
      const response = await fetch(`http://localhost:5000/api/crop-details/${cropName}?state=${encodeURIComponent(state)}&district=${encodeURIComponent(district)}`, {
        headers: {
          'Authorization': `Bearer ${token}`,
          'Content-Type': 'application/json'
        }
  ...
  const loadCropDetails = async () => {
    try {
      ...
      const response = await fetch(`http://localhost:5000/api/crop-details/${cropName}${locationParams}`, {
        headers: {
          'Authorization': `Bearer ${token}`,
          'Content-Type': 'application/json'
        }
